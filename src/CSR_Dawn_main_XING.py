...
def main(trial=0):
    ...
    eval_batch_size = 100000  # the batch size when test

    dat = load_data_XING.load_data(data_name)  # function description is found at the next of main()
    ...
    test_eval = dat['test_item_eval']
    validation_eval = dat['vali_item_eval']

    user_list = dat['user_list']
    item_list = dat['item_list']
    item_warm = np.unique(item_list)
    v_neighbor_mat = np.load('../data/XING/top_k_nn_item_index.npy')
    ...
    item_content_vec = item_content_vec.toarray()
    ...
    csr_dawn = CSR_Dawn(cf_vec_rank=cf_vec_rank,
                        ...
                        data_set_name="XING", model_dir="../model", trials=trial, seed=seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--decay_lr_every', type=int, default=3, help='decay step')
